...
class Supervisor extends CI_Controller
{
    ...
    public function mpk()
    {
        ...
        $this->load->view('supervisor/script');
    ...
    public function notif()
    {
        ...
        $data = $this->supervisor_model->getNotif();
        if ($data->num_rows() > 0) {
            ...
            $output .= '
                <script>
                    $(".updateNotif").on("click", function() {
                        let id = $(this).data("id");
                        $.ajax({
                            url: "' . base_url('/supervisor/updateNotif/') . '" + id,
                            method: "POST",
                            data: {
                                "notif": id
                            }
                    ...
                    $(".updateNotifLink").on("click", function() {
                        ...
                        document.location.href = "' . base_url('supervisor/detail/') . '" + id;
                    });
                </script>
            ';
        } else {
    ...
    {
        $this->supervisor_model->notifUpdate($id);
    }
}
